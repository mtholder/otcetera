...
void add_basic_node_info(const RichTaxonomy & taxonomy, const SumTreeNode_t & nd, json & noderepr, bool is_arguson = false) {
    ...
    noderepr["num_tips"] = (nd.is_tip() ? 0 : nd.get_data().num_tips);
...
const SumTreeNode_t * find_node_by_id_str(const std::unordered_map<OttId, OttId> & forwards,
    ...
    if (std::regex_match(node_id, matches, mrca_id_pattern)) {
        ...
        bool bogus = false;
        auto fir_nd = find_node_by_id_str(forwards, tree, first_id, bogus);
        if (fir_nd == nullptr) {
            return nullptr;
        }
        auto sec_nd = find_node_by_id_str(forwards, tree, second_id, bogus);
        if (sec_nd == nullptr) {
            return nullptr;
        }
...
const SumTreeNode_t * find_required_node_by_id_str(const std::unordered_map<OttId, OttId> & forwards,
                                                   const SummaryTree_t & tree,
                                                   const string & node_id,
                                                   bool & was_broken) {
    auto node  = find_node_by_id_str(forwards, tree, node_id, was_broken);
    if (not node) {
...
string node_info_ws_method(const TreesToServe & tts,
    ...
    set<string> usedSrcIds;
    {
        auto locked_taxonomy = tts.get_readable_taxonomy();
        const auto & taxonomy = locked_taxonomy.first;
        const SumTreeNode_t * focal = find_required_node_by_id_str(taxonomy.get_forwards_map(), *tree_ptr, node_id, was_broken);
...
string mrca_ws_method(const TreesToServe & tts,
    ...
    for (auto node_id : node_id_vec) {
        const SumTreeNode_t * n =  nullptr; 
        {
            auto locked_taxonomy = tts.get_readable_taxonomy();
            const auto & taxonomy = locked_taxonomy.first;
            n = find_required_node_by_id_str(taxonomy.get_forwards_map(), *tree_ptr, node_id, was_broken);
        }
        if (first) {
            first = false;
            focal = n;
...
const SumTreeNode_t * get_node_for_subtree(const SummaryTree_t * tree_ptr,
    ...
    const SumTreeNode_t * focal = nullptr;
    {
        auto locked_taxonomy = tts.get_readable_taxonomy();
        const auto & taxonomy = locked_taxonomy.first;
        focal =  find_required_node_by_id_str(taxonomy.get_forwards_map(), *tree_ptr, node_id, was_broken);
    }
    if (was_broken) {
...
vector<OttId> extra_children_for_node(OttId id, const SummaryTree_t& summary, const RichTaxonomy& taxonomy) {
    ...
    for(unsigned int i=0;i<bad_parents.size();i++) {
